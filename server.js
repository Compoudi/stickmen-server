...
wss.on("connection", (ws) => {
  ...
  // 🚫 Si la room est fermée (ou marquée terminée), on refuse
  if (!room || room.closed) {
    ws.send(JSON.stringify({ type: "roomClosed" }));
    ws.close();
    console.warn("⚠️ Tentative de rejoindre une room fermée :", roomId);
    return;
  }
  ...
  ws.on("message", (msg) => {
    ...
    // 🚪 Exit → ferme définitivement la room
    if (data.type === "exitGame") {
      console.log(`🚪 Fermeture définitive de la room ${room.id}`);
      room.closed = true; // 🔒 empêche tout nouvel accès
      for (const pl of room.players) {
        if (pl.ws.readyState === 1)
          pl.ws.send(JSON.stringify({ type: "goToMenu" }));
      }
      // optionnel : nettoyage mémoire
